...
// ═══════════════════════════════════════════════════════════════
// ZELREX ANALYTICS — ONE ENDPOINT TO RULE THEM ALL
//
// GET /api/z/px?u=userId&t=pv&...  → pixel tracking (1x1 GIF)
// GET /api/z/px?action=dash&userId=xxx&range=30d  → dashboard JSON
//
// This is the ONLY file needed for analytics. Nothing else.
// ═══════════════════════════════════════════════════════════════
...
} catch (e) { console.error('[ZPX] Supabase init failed'); }
...
export async function GET(req: Request) {
  ...
  if (!userId || !code || !supabase || !EVENTS[code]) {
  ...
  supabase.from('site_analytics').insert({
    ...
    element_id: extra.slice(0, 200),
    element_text: extra.slice(0, 200),
    visitor_id: visitorId.slice(0, 100),
    referrer: referrer.slice(0, 500),
    device_type: devMap[devCode] || 'desktop',
    country,
    metadata,
  }).then(({ error }: any) => {
    if (error) console.error('[ZPX] INSERT ERR:', error.message);
  });
...
async function handleDash(url: URL) {
  ...
  try {
    ...
    const ccData = ccR.data || [];
    const csData = csR.data || [];
    const tpData = tpR.data || [];
    const allData = allR.data || [];
    const revData = revR.data || [];

    const pageviews = pvData.length;
    const uniqueVisitors = new Set(pvData.map((r: any) => r.visitor_id)).size;
    const ctaClicks = ccData.length;
    const checkoutStarts = csData.length;

    // Avg time
    const times = tpData.map((r: any) => {
    ...
    pvData.forEach((r: any) => { const p = r.page_path || '/'; pc[p] = (pc[p] || 0) + 1; });
    ...
    pvData.forEach((r: any) => {
      ...
      try { const h = new URL(r.referrer).hostname; if (h) rc[h] = (rc[h] || 0) + 1; } catch {}
    });
    const topReferrers = Object.entries(rc).sort(([, a], [, b]) => (b as number) - (a as number)).slice(0, 10).map(([referrer, count]) => ({ referrer, count }));

    // Devices
    const dc: Record<string, number> = {};
    pvData.forEach((r: any) => { const d = r.device_type || 'unknown'; dc[d] = (dc[d] || 0) + 1; });
    ...
    allData.forEach((r: any) => {
      const day = r.created_at?.slice(0, 10); if (!day) return;
    ...
    revData.forEach((r: any) => {
      ...
      tierMap[t].total += r.amount_cents || 0; tierMap[t].count++;
    });
    ...
    revData.forEach((r: any) => {
      ...
      dRevMap[day].amount += r.amount_cents || 0; dRevMap[day].count++;
    });
    const dailyRevenue = Object.entries(dRevMap).sort(([a], [b]) => a.localeCompare(b)).map(([date, d]) => ({ date, ...d }));

    return NextResponse.json({
      timeRange: range, pageviews, uniqueVisitors, ctaClicks, checkoutStarts,
      ...
      avgTimeOnPage, topPages, topReferrers, deviceBreakdown, dailyData,
      revenue: { total: revTotal, count: revCount, avgOrder, byTier, dailyRevenue },
    }, { headers: jh });
  } catch (e: any) {
    console.error('[ZPX] DASH ERR:', e?.message);
    return NextResponse.json({ pageviews: 0 }, { headers: jh });
  }
}
