...
function wantsWebsite(message: string) {
  return /build.*website|make.*website|create.*website/i.test(message);
}
...
const shouldLaunchBusiness =
  ...
  (
    ...
    // Early shortcut: if the user explicitly asks to build a website, generate and save one
    if (
      lastUserMessage &&
      typeof lastUserMessage.content === "string" &&
      wantsWebsite(lastUserMessage.content)
    ) {
      ...
      const website = buildWebsite({
        ...
        branding: {
          name: "DemoCo",
          tagline: "A modern business built for results",
          tone: "professional",
          primaryColor: "#4F8CFF",
        },
      });
