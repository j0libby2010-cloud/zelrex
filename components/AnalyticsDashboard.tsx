...
export function AnalyticsDashboard({
  ...
  const formatNumber = (n: number) => n >= 10000 ? `${(n / 1000).toFixed(1)}k` : n.toLocaleString();
  ...
  return (
    <div style={{
      ...
      animation: "dashFadeIn 300ms ease forwards",
      overflow: "hidden",
    }}>
      <style>{`
        @keyframes dashFadeIn { from { opacity: 0; } to { opacity: 1; } }
        @keyframes shimmer { 0% { background-position: -200% 0; } 100% { background-position: 200% 0; } }
        .stat-card:hover { transform: translateY(-2px); box-shadow: 0 12px 40px rgba(0,0,0,0.4), inset 0 0.5px 0 rgba(255,255,255,0.12) !important; }
        .stat-card { transition: transform 200ms ease, box-shadow 200ms ease; }
        .range-btn { transition: all 200ms ease; }
        .range-btn:hover { background: rgba(255,255,255,0.06) !important; }
        .chart-tab { transition: all 200ms ease; cursor: pointer; }
        .chart-tab:hover { background: rgba(255,255,255,0.04) !important; }
        .glass-scroll::-webkit-scrollbar { width: 6px; }
        .glass-scroll::-webkit-scrollbar-track { background: transparent; }
        .glass-scroll::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 999px; }
        .glass-scroll::-webkit-scrollbar-thumb:hover { background: rgba(255,255,255,0.18); }
      `}</style>
      ...
      <div style={{
            ...
            <button
              ...
              style={{
                ...
                background: range === r.key
                  ? `linear-gradient(135deg, ${G.accent}25, ${G.accent}10)`
                  : "transparent",
                color: range === r.key ? G.accent : G.textSec,
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 999,
            display: "flex", alignItems: "center", justifyContent: "center",
            color: G.textSec, cursor: "pointer", border: `1px solid ${G.glassBorder}`,
            background: G.glass,
          }}
        >
          ✕
      ...
      <div className="glass-scroll" style={{ flex: 1, overflow: "auto", padding: 20 }}>
        {loading && !data ? (
          ...
          <div style={{ textAlign: "center", padding: 60, color: G.textSec }}>
            <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8 }}>No data yet</div>
            <div style={{ fontSize: 13 }}>Deploy your website to start collecting analytics.</div>
          </div>
        ) : (
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            {/* ─── Stat Cards Row ────────────────────────── */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 20 }}>
              <StatCard label="Visitors" value={formatNumber(data.uniqueVisitors)} sub={`${formatNumber(data.pageviews)} pageviews`} color={G.accent} glow={G.accentGlow} />
              <StatCard label="CTA Clicks" value={formatNumber(data.ctaClicks)} sub={`${data.conversionRate}% click rate`} color={G.green} glow={G.greenGlow} />
              <StatCard label="Checkouts" value={formatNumber(data.checkoutStarts)} sub={`${data.checkoutRate}% checkout rate`} color={G.amber} glow={G.amberGlow} />
              <StatCard label="Revenue" value={formatCurrency(data.revenue.total)} sub={`${data.revenue.count} payments`} color={G.purple} glow={G.purpleGlow} />
            </div>

            {/* ─── Chart Section ─────────────────────────── */}
            <div className="stat-card" style={{ ...liquidGlass, padding: 0, marginBottom: 20 }}>
              {/* Chart tabs */}
                    ...
                    <AreaChart data={data.dailyData}>
                      <defs>
                        <linearGradient id="gradViews" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor={G.chartLine1} stopOpacity={0.3} />
                          <stop offset="100%" stopColor={G.chartLine1} stopOpacity={0} />
                        </linearGradient>
                        <linearGradient id="gradClicks" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor={G.chartLine2} stopOpacity={0.2} />
                      ...
                      <Area type="monotone" dataKey="clicks" stroke={G.chartLine2} strokeWidth={2} fill="url(#gradClicks)" name="CTA Clicks" dot={false} />
              ...
              {activeChart === "traffic" && (
                <div style={{ display: "flex", gap: 20, padding: "0 20px 14px", justifyContent: "center" }}>
                  <LegendDot color={G.chartLine1} label="Pageviews" />
                  <LegendDot color={G.chartLine2} label="CTA Clicks" />
                  <LegendDot color={G.chartLine3} label="Checkouts" dashed />
                </div>
              )}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
              {/* Top Pages */}
              <div className="stat-card" style={{ ...liquidGlass, padding: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: G.text, marginBottom: 14 }}>Top Pages</div>
                {data.topPages.length === 0 ? (
                  <div style={{ fontSize: 12, color: G.textMuted }}>No page data yet</div>
                ) : (
                  data.topPages.slice(0, 6).map((p, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < data.topPages.length - 1 ? `1px solid ${G.glassBorder}` : "none" }}>
                      <span style={{ fontSize: 13, color: G.textSec, fontFamily: "monospace" }}>{p.path}</span>
                      <span style={{ fontSize: 13, fontWeight: 600, color: G.text }}>{p.views}</span>
                    </div>
                  ))
                )}
              </div>
              ...
              {/* Top Referrers */}
              <div className="stat-card" style={{ ...liquidGlass, padding: 20 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: G.text, marginBottom: 14 }}>Top Referrers</div>
                {data.topReferrers.length === 0 ? (
                  <div style={{ fontSize: 12, color: G.textMuted }}>No referrer data yet</div>
                ) : (
                  data.topReferrers.slice(0, 6).map((r, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: i < data.topReferrers.length - 1 ? `1px solid ${G.glassBorder}` : "none" }}>
                      <span style={{ fontSize: 13, color: G.textSec }}>{r.referrer}</span>
                      <span style={{ fontSize: 13, fontWeight: 600, color: G.text }}>{r.count}</span>
                    </div>
                  ))
                )}
              </div>
              ...
              <div className="stat-card" style={{ ...liquidGlass, padding: 20 }}>
                ...
                {data.deviceBreakdown.length === 0 ? (
                  <div style={{ fontSize: 12, color: G.textMuted }}>No device data yet</div>
                ) : (
                    ...
                    {data.deviceBreakdown.map((d, i) => (
                      <div key={i}>
                        ...
                        <div style={{ height: 4, borderRadius: 999, background: G.glassBorder }}>
                          <div style={{
                            height: "100%", borderRadius: 999, width: `${d.pct}%`,
            ...
            {data.revenue.byTier.length > 0 && (
              <div className="stat-card" style={{ ...liquidGlass, padding: 20, marginTop: 14 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(data.revenue.byTier.length, 4)}, 1fr)`, gap: 14 }}>
                  {data.revenue.byTier.map((t, i) => (
                    <div key={i} style={{
                      ...
                      background: `linear-gradient(135deg, ${PIE_COLORS[i % PIE_COLORS.length]}10, transparent)`,
                      border: `1px solid ${PIE_COLORS[i % PIE_COLORS.length]}20`,
                    }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}>
              <div className="stat-card" style={{ ...liquidGlass, padding: 20, display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: G.accentGlow, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                    <circle cx="12" cy="12" r="10" stroke={G.accent} strokeWidth="1.4" />
                    <path d="M12 6v6l4 2" stroke={G.accent} strokeWidth="1.6" strokeLinecap="round" />
                  </svg>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: G.textSec, marginBottom: 2 }}>Avg. Time on Page</div>
                  <div style={{ fontSize: 24, fontWeight: 800, color: G.text, letterSpacing: "-0.02em" }}>{formatTime(data.avgTimeOnPage)}</div>
                </div>
              </div>
              <div className="stat-card" style={{ ...liquidGlass, padding: 20, display: "flex", alignItems: "center", gap: 16 }}>
                <div style={{ width: 48, height: 48, borderRadius: 14, background: data.revenue.count > 0 ? G.greenGlow : G.amberGlow, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
                    <rect x="2" y="5" width="20" height="14" rx="2" stroke={data.revenue.count > 0 ? G.green : G.amber} strokeWidth="1.4" />
                    <path d="M2 10h20" stroke={data.revenue.count > 0 ? G.green : G.amber} strokeWidth="1.4" />
                  </svg>
                </div>
                <div>
                  <div style={{ fontSize: 12, color: G.textSec, marginBottom: 2 }}>Avg. Order Value</div>
                  <div style={{ fontSize: 24, fontWeight: 800, color: G.text, letterSpacing: "-0.02em" }}>
                    {data.revenue.count > 0 ? formatCurrency(data.revenue.avgOrder) : "—"}
                  </div>
                </div>
              </div>
            </div>
...
function LegendDot({ color, label, dashed }: { color: string; label: string; dashed?: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: G.textSec }}>
      <div style={{
        width: 16, height: 2, background: color, borderRadius: 999,
...
function CustomTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontWeight: 600, color: G.text, marginBottom: 6 }}>
      ...
      {payload.map((p: any, i: number) => (
        <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
          ...
          <span style={{ color: G.textSec }}>{p.name}:</span>
...
function RevenueTooltip({ active, payload, label }: any) {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: G.textMuted, fontSize: 11, marginTop: 2 }}>
        {payload[0]?.payload?.count || 0} payments
      </div>
    </div>
  );
}
